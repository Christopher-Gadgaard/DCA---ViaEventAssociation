...
public class ViaEventUpdateDescriptionCommand
{
    ...
    public static OperationResult<ViaEventUpdateDescriptionCommand> Create(string id, string description)
    {
        ...
        var combinedResult =
            OperationResult<ViaEventUpdateDescriptionCommand>.Combine(eventId.OperationErrors,
                eventDescription.OperationErrors);
        return combinedResult.OperationErrors.Count > 0
            ? combinedResult
            : new ViaEventUpdateDescriptionCommand(eventId.Payload, eventDescription.Payload);
    }

    private ViaEventUpdateDescriptionCommand(ViaEventId id, ViaEventDescription description) =>
